...
public class PokerTableController {
	...
	private void initialize() {
		btnShowWinner.setVisible(false);
	...
	public void handleShowWinner() {
		// Show the Winner
		GamePlayPlayerHand WinningGPPH = gme.getWinningGPPH();
		Player WinningPlayer = WinningGPPH.getWinningPlayer();
		Hand WinningHand = WinningGPPH.getBestHand();
		WinnerTxt.setVisible(true);
		String my_text = "The Winner is  " + String.valueOf(WinningPlayer.getPlayerName());
		WinnerTxt.setText(my_text);

		System.out.println(WinningHand.getCards().size());
		System.out.println(WinningHand.getHandStrength());

		switch (WinningPlayer.getiPlayerPosition()) {
		case 1:
			for (int i = 0; i < gme.getNbrOfCards(); i++) {
				ImageView CardImage = (ImageView) hBoxP1Cards.getChildren().get(i);
				shift_card(hBoxP1Cards, CardImage).play();
			}
			/*
			 * TranslateWinningCards(WinningHand, WinningPlayer, hBoxP1Cards);
			 */
			break;
		case 2:
			TranslateWinningCards(WinningHand, WinningPlayer, hBoxP2Cards);
			break;

		case 3:
			TranslateWinningCards(WinningHand, WinningPlayer, hBoxP3Cards);
			break;

		case 4:
			TranslateWinningCards(WinningHand, WinningPlayer, hBoxP4Cards);
			break;
	...
	private void handlePlay() {
		this.eGameState = eGameState.StartOfGame;

		if (WinningCardsArea != null) {
			WinningCardsArea.getChildren().clear();
		}
		...
		// Get the Rule, Add a deck, start the Game

		if (RootLayoutController.getClickedRule() != null) {
			gme = new GamePlay(RootLayoutController.getClickedRule());
			gme.setGameDeck(new Deck(RootLayoutController.getClickedRule().GetNumberOfJokers(),
					RootLayoutController.getClickedRule().GetRuleCards()));
		}

		else {
			Rule default_rule = (new Rule(pokerEnums.eGame.FiveStud));
			gme = new GamePlay(default_rule);
			gme.setGameDeck(new Deck(default_rule.GetNumberOfJokers(), default_rule.GetRuleCards()));
	...
	private void handleDraw() {
		...
		if (act.geteDrawAction() == eDrawAction.DrawPlayer) {
			...
			for (int iDraw = 0; iDraw < act.getiCardDrawn(); iDraw++) {
				...
				for (Player p : mainApp.GetSeatedPlayers()) {
					...
					gme.FindPlayerGame(gme, p).addCardToOriginalHand(c);
		...
		if (act.isbEvaluateHand()) {
			...
			GamePlayPlayerHand WinningGPPH = new GamePlayPlayerHand();
			WinningGPPH.setBestHand(WinningHand);
			WinningGPPH.setGame(gme);
			WinningGPPH.setPlayer(WinningPlayer);
			WinningGPPH.setWinningPlayer(WinningPlayer);
			WinningGPPH.setHand(gme.FindPlayerGame(gme, WinningPlayer).getHand());

			gme.setWinningGPPH(WinningGPPH);
			System.out.println("Winning Player Position: " + WinningPlayer.getiPlayerPosition());
			SetGameControls(eGameState.EndOfGame);

			/*
			 * ArrayList<CardDomainModel> WinningCards = WinningHand.getCards();
			 * for (int i = 0; i < 5; i++) { String my_card_string =
			 * "src/main/resources/res/img/" + WinningCards.get(i).getCardImg();
			 * System.out.println(my_card_string); Image my_image = new
			 * Image(getClass().getResourceAsStream(my_card_string), 75, 75,
			 * true, true); ImageView my_image_view = new ImageView(my_image);
			 * WinningCardsArea.getChildren().add(my_image_view); }
			 * 
			 * //Show the winning hand for (CardDomainModel c :
			 * WinningHand.getCards()) { String strCard = "/res/img/" +
			 * c.getCardImg(); System.out.print(strCard); Image my_image = new
			 * Image(getClass().getResourceAsStream(strCard), 75, 75, true,
			 * true); ImageView my_image_view = new ImageView(my_image);
			 * WinningCardsArea.getChildren().add(my_image_view); }
			 */
	...
	// Lab 5 WinningCards Translation
	private void TranslateWinningCards(Hand WinningHand, Player WinningPlayer, HBox my_hbox) {
		GamePlayPlayerHand Player_GPPH = gme.FindPlayerGame(gme, WinningPlayer);
		GamePlayPlayerHand Common_GPPH = gme.FindCommonHand(gme);

		switch (WinningHand.getbNatural()) {

		case 1:
			if (gme.getRule().GetCommunityCardsCount() == 0) {
				for (int i = 0; i < Player_GPPH.getOrignalHand().getCards().size(); i++) {
					shift_card(my_hbox, my_hbox.getChildren().get(i));
				}
			} else {
				for (int i = 0; i < Player_GPPH.getOrignalHand().getCards().size(); i++) {

					for (CardDomainModel winning_card : WinningHand.getCards()) {

						if (Player_GPPH.getOrignalHand().getCards().get(i).getRank() == winning_card.getRank()
								&& Player_GPPH.getOrignalHand().getCards().get(i).getSuit() == winning_card.getSuit()) {
				...
				for (int i = 0; i < Common_GPPH.getOrignalHand().getCards().size(); i++) {

					Boolean Card_is_WinningCard = false;

					for (CardDomainModel winning_card : WinningHand.getCards()) {

						if (Common_GPPH.getOrignalHand().getCards().get(i).getRank() == winning_card.getRank()
								&& Common_GPPH.getOrignalHand().getCards().get(i).getSuit() == winning_card.getSuit()) {
							shift_card(HboxCommunityCards, HboxCommunityCards.getChildren().get(i));
						}
					}
				}
			}
		...
		case 0:
			...
			if (gme.getRule().GetCommunityCardsCount() == 0) {
				...
				for (int i = 0; i < Player_GPPH.getOrignalHand().getCards().size(); i++) {
					...
					if (Player_GPPH.getOrignalHand().getCards().get(i).getWild()) {
						shift_card(my_hbox, my_hbox.getChildren().get(i));
						continue;
					}
	...
	private TranslateTransition shift_card(HBox my_hbox, Node hbox_image) {

		Bounds bndCardDealt = hbox_image.localToScene(hbox_image.getBoundsInLocal());
		Point2D pntCardDealt = new Point2D(bndCardDealt.getMinX(), bndCardDealt.getMinY());

		TranslateTransition translate_card = new TranslateTransition();
		translate_card.setFromX(0);
		translate_card.setToX(pntCardDealt.getX());
		translate_card.setFromY(0);
		translate_card.setToY(pntCardDealt.getY() + 20);
		translate_card.setCycleCount(1);
		translate_card.setAutoReverse(false);

		return translate_card;
	}
